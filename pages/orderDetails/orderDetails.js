...
Page({
  ...
  init(id) {
    ...
    util.http('order/detail', { id: id }, 'post',token).then(res => {
      if (res.code == 200) {
        this.setData({
          info: res.data,
  ...
  toServiceType(e) {
    ...
    if (status != 2) {
      wx.navigateTo({
        url: '../serviceType/serviceType?type=' + type + '&id=' + id + '&order_id=' + order_id
      })
    } else {
      wx.navigateTo({
        url: '../refund/refund?type=' + type + '&id=' + id 
      })
    }
  },
  detailsRefund(e) {
    main.toDetails(e, "detailsRefund")
  },
  details(e) {
  ...
  pay(e) {
    ...
    let token = app.globalData.token;
    let that = this;
    util.http("pay/pay", { id: id }, 'post', token).then(res => {
      wx.requestPayment({
